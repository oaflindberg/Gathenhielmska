<?php wp_footer(); ?>

<footer>

  <div class="footer-wrapper">

    <img class="footer-img" src="<?php bloginfo('template_directory') ?>/assets/images/background/footer-img.png" alt="bird">

    <div class="desktop-wrapper">
      <div class="contact-us">
        <div>
          <h4>Kontakta oss!</h4>
        </div>
        <div>
          <p>Stigbergstorget 7 </p>
          <p>414 63 Göteborg</p>
        </div>
        <div>
          <p>gathenhielmskabokning@gmail.com</p>
        </div>

      </div>

      <div class="follow-us">
        <h4>Följ oss</h4>
        <div class="follow-wrapper">
          <a href="https://www.facebook.com/gathenhielmska/"> <img src="<?php bloginfo('template_directory') ?>/assets/images/icons/facebook.svg" alt="facebook"> </a>
          <a href="https://www.instagram.com/"> <img src="<?php bloginfo('template_directory') ?>/assets/images/icons/instagram.svg" alt="instagram"> </a>
          <a href="https://www.youtube.com/"> <img src="<?php bloginfo('template_directory') ?>/assets/images/icons/youtube.svg" alt="youtube"> </a>
        </div>
      </div>
    </div>
    <div class="sponsor-us">
      <h4>Sponsorer</h4>

      <div class="sponsor-wrapper">
        <img class="spons1" src="<?php bloginfo('template_directory') ?>/assets/images/icons/sponsor1.png" alt="sponsor">
        <img class="spons2" src="<?php bloginfo('template_directory') ?>/assets/images/icons/sponsor2.png" alt="sponsor">
        <img class="spons3" src="<?php bloginfo('template_directory') ?>/assets/images/icons/sponsor3.png" alt="sponsor">

      </div>
    </div>

  </div>

</footer>
</body>

</html>